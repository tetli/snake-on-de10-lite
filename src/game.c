...
{   
    ...
    if(isoDrawMode){
        for (int i = -1; i < MAP_XWIDTH; i++){
            for (int j = -1; j < MAP_YHEIGHT; j++){ 
                if(i==-1 || j == -1) draw_cube_to_screen(i, j, 0x00, 0x49);
            } 
        }
    } 
    ...
    for (int i = 0; i < MAP_XWIDTH; i++)
    {
        for (int j = 0; j < MAP_YHEIGHT; j++)
        {
            ...
            case 0:
                // draw_floor_tile(i, j);
                if (isoDrawMode) draw_tile_to_screen(i, j, 0x00, 0x92);
                else draw_rect_to_screen(i, j, 0x49);
                break;
            case 1:
                // draw_snake_head(i, j);
                if (isoDrawMode) draw_cube_to_screen(i, j, 0x0C, 0x10);
                else draw_rect_to_screen(i, j, 0x0C);
                break;
            case 2:
                // draw_snake_tail(i, j);
                if (isoDrawMode) draw_cube_to_screen(i, j, 0x0C, 0x14);
                else draw_rect_to_screen(i, j, 0x10);
                break;
            case 3:
                // draw_fruit(i, j);
                if (isoDrawMode) draw_cube_to_screen(i, j, 0xA0, 0xC0);
                else draw_rect_to_screen(i, j, 0xC0);
                break;
            default:
                break;
            }
